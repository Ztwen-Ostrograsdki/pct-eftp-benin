            ...
            <div class="text-left w-full">
              <div class="relative flex flex-col">
                ...
                <h4 class="text-2xl font-bold dark:text-gray-200"> 
                    ...
                    <span class="text-gray-400 float-right text-sm "> {{ numberZeroFormattor(count($orders), true) }} </span>
                </h4>
                  ...
                  <div>
                    <a title="Charger le profil de {{ $order->user->getFullName() }}" href="{{ route('user.profil', ['identifiant' => $order->user->identifiant]) }}">
                        ...
                        <p class="text-xs text-gray-500 dark:text-gray-400">
                            {{ $order->user->current_function }}
                        </p>
                    </a>
                  </div>
                </div>
                <p class="px-6 text-base font-medium text-gray-600 dark:text-gray-400"> Inscrit depuis, {{ $order->user->__getDateAsString($order->user->created_at) }}
                </p>
              </div>
                ...
                <div class="flex items-center px-6 space-x-1 text-sm font-medium text-gray-500 dark:text-gray-400">
                  <div class="flex items-center">
                    <div class="flex gap-x-2 mr-3 text-sm text-gray-700 dark:text-gray-400">
                        <div>
                            <span wire:click="confirmedUserUnblocked({{$order->id}})" class="border cursor-pointer bg-green-300 text-green-700 hover:bg-green-400 px-3 py-2 rounded ">
                      ...
                      <div>
                        <span wire:click="deleteNotif({{$order->id}})" class="border cursor-pointer bg-red-300 text-red-700 hover:bg-red-400 px-3 py-2 rounded ">
                            <span class="fas fa-trash"></span>
                            <span wire:loading.remove wire:target='deleteNotif({{$order->id}})'>Supprimer</span>
                            <span wire:loading wire:target='deleteNotif({{$order->id}})' class="fas fa-rotate animate-spin"></span>
                        </span>
                      </div>
                      <div>
