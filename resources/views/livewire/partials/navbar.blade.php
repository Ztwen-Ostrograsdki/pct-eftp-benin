        ...
        <div id="navbar-collapse-with-animation" class="hs-collapse hidden overflow-hidden transition-all duration-300 basis-full grow md:block">
          <div class="overflow-hidden overflow-y-auto max-h-[75vh] [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-track]:bg-gray-100 [&::-webkit-scrollbar-thumb]:bg-gray-300 dark:[&::-webkit-scrollbar-track]:bg-slate-700 dark:[&::-webkit-scrollbar-thumb]:bg-slate-500">
            <div class="flex flex-col gap-x-0 mt-5 divide-y divide-dashed divide-gray-200 md:flex-row md:items-center md:justify-end md:gap-x-7 md:mt-0 md:ps-7 md:divide-y-0 md:divide-solid dark:divide-gray-700">
              ...
              <a class=" hover:text-gray-500 font-medium {{request()->is('/') ? 'text-blue-600' : 'text-gray-400' }} py-3 md:py-6 dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600" href="/" aria-current="page">Acceuil</a>
              
              <a class="font-medium hover:text-gray-500 py-3 md:py-6 {{request()->route()->named('master.members.home') ? 'text-blue-600' : 'text-gray-400' }} dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600" href="{{route('master.members.home')}}">
                L'association
              </a>

              <a class="font-medium hover:text-gray-500 py-3 md:py-6 {{request()->route()->named('master.users.list') ? 'text-blue-600' : 'text-gray-400' }} dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600" href="{{route('master.users.list')}}">
                Les utilisateurs
              </a>

              <a class="font-medium hover:text-gray-500 py-3 md:py-6 {{request()->route()->named('master.orders.list') ? 'text-blue-600' : 'text-gray-400' }} dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600" href="{{route('master.orders.list')}}">
                Les commandes
              </a>

              <a id="my-tooltip" title="Télécharger des épreuves" class="transition duration-150 ease-in-out dark:focus:text-primary-500 dark:active:text-primary-600 font-medium text-gray-400 hover:text-gray-500 py-3 md:py-6  dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600" href="{{route('library.home')}}">
                Bibliothèque
              </a>
  
              <a href="{{route('shopping.home')}}" class="font-medium hover:text-gray-500 py-3 md:py-6 {{request()->route()->named('shopping.home') ? 'text-blue-600' : 'text-gray-400' }}  dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600">
                Librairie
              </a>
              @if(auth_user() && count(auth()->user()->getNotifications()))
              <a href="{{route('user.notifications')}}" class="font-medium hover:text-gray-500 py-3 md:py-6 {{request()->route()->named('user.notifications') ? 'text-blue-600' : 'text-gray-400' }} dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600">
                Notifications
              ...
              <a class="font-medium flex items-center hover:text-gray-500 py-3 md:py-6 {{request()->route()->named('shopping.cart') ? 'text-blue-600' : 'text-gray-400' }} dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600" href="{{route('user.cart', ['identifiant' => auth_user()->identifiant])}}">
              ...
              <div class="hs-dropdown-menu transition-[opacity,margin] duration-[0.1ms] md:duration-[150ms] hs-dropdown-open:opacity-100 opacity-0 md:w-48 hidden z-10 bg-white md:shadow-md rounded-lg p-2 dark:bg-gray-800 md:dark:border dark:border-gray-700 dark:divide-gray-700 before:absolute top-full md:border before:-top-5 before:start-0 before:w-full before:h-5">
                ...
                @endif
                <a class="flex items-center gap-x-3.5 py-2 px-3 rounded-lg text-sm text-gray-800 hover:bg-gray-100 focus:ring-2 focus:ring-blue-500 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-gray-300 dark:focus:outline-none dark:focus:ring-1 " href="{{route('user.orders', ['identifiant' => auth_user()->identifiant])}}">
                  Mes commandes
                </a>
                ...
                @endif
                <a href="{{route('user.profil', ['identifiant' => auth_user()->identifiant])}}" class="flex items-center gap-x-3.5 py-2 px-3 rounded-lg text-sm text-gray-800 hover:bg-gray-100 focus:ring-2 focus:ring-blue-500 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-gray-300 dark:focus:outline-none dark:focus:ring-1">
                  Mon profil
                </a>
